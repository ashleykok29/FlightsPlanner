// Purpose: To log in / register a user that has already scheduled a trip 
// Team: 39
    ...
    if(password === confirmPassword) // Checking that passwords match
    {
       if(password.length >= 8) // Checking the password is greater than or equal to 8 characters long
       {
            ...
            if(emailIsNew) // If the email is new and not previously used
            {
                ...
                let currentUserIndex = localStorage.getItem(CURRENT_USER_KEY); // Getting the user index
                
                // The following code adds anon's one trip to the new user, then deletes it from anon.
                listOfUsers.users[currentUserIndex].trips.push(listOfUsers.users[0].trips[0]);
                listOfUsers.users[0].trips.pop();
            
                updateLocalStorage(listOfUsers,USERS_DATA_KEY); // Updating local storage
                window.location = "scheduledTrips.html"; // Directing the user to scheduled trips to view their trip 
            }
...
function login()
{
    ...
    if(userPassword === listOfUsers.getUser(foundUserIndex).password) // If the entered password is the same as the user's saved password
    {
        ...
        // The following code adds anon's one trip to the new user, then deletes it from anon
        listOfUsers.users[foundUserIndex].trips.push(listOfUsers.users[0].trips[0]);
        listOfUsers.users[0].trips.pop();
        updateLocalStorage(listOfUsers,USERS_DATA_KEY); // Updating the local storage
        
        window.location = "scheduledTrips.html"; // Directing the user to scheduled trips
